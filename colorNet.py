...
class myNet_ocr(nn.Module):
    def __init__(self,cfg=None,num_classes=78,export=False,trt=False,is_color=False):
        ...
        if self.is_color:
            ...
            self.color_classifier=nn.Conv2d(num_classes,5,kernel_size=1,stride=1)
...
if __name__ == '__main__':
    ...
    ckpt = torch.load(r"E:/study/plate/yolov7_plate-master/weights/plate_rec_big.pth")
    model = myNet_ocr(num_classes=78,export=True,is_color=True)
    model.load_state_dict(ckpt['state_dict'],strict=False)
    for key,value in model.named_parameters():
        print(key)
    # print(model)
    out,x_color= model(x)
    print(out.shape,x_color.shape)